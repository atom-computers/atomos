        ...
        match event {
            ...
            InputEvent::TouchDown { event, .. } => {
                ...
                if let Some(seat) = shell.seats.for_device(&event.device()).cloned() {
                    ...
                    // --- ATOMOS EDGE SWIPE INTERCEPTOR ---
                    let output_geo = output.geometry();
                    if position.y >= (output_geo.size.h as f64 - 30.0) {
                        tracing::info!("AtomOS: Bottom edge swipe-up gesture started!");
                        // We swallow the event here so it is not forwarded to the active app.
                        // We would also set a `bottom_edge_swipe_slot = Some(event.slot())`
                        // in the shell state so TouchMotion knows to scale the window.
                        return;
                    }
                    // -------------------------------------
