...
impl eframe::App for PreviewApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        let base = backdrop_color();
        let bg_color = egui::Color32::from_rgba_premultiplied(
            ((base.r() as f32) * anim) as u8,
            ((base.g() as f32) * anim) as u8,
            ((base.b() as f32) * anim) as u8,
            alpha,
        );

        egui::Area::new(egui::Id::new("app_handler_area"))
            .order(egui::Order::Background)
            .show(ctx, |ui| {
                egui::Frame::NONE.fill(bg_color).inner_margin(0.0).show(ui, |ui| {
