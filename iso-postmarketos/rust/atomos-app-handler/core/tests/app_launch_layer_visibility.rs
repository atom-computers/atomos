...
//! orchestrates home fold/unfold or chat-ui layer promotion.
...
#[test]
fn run_launch_once_does_not_call_home_ipc() {
    assert!(
        !LINUX_RS.contains("apply_home_ipc"),
        "run_launch_once must not call home fold/unfold D-Bus",
    );
    assert!(
...
#[test]
fn on_toplevel_count_changed_does_not_call_home_fold() {
    assert!(
        !LINUX_RS.contains("derive_home_ipc"),
        "on_toplevel_count_changed must not call home fold/unfold IPC",
    );
}