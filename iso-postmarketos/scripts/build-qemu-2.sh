...
BASE_APK_PACKAGES=(
    ...
    # greetd display manager + wlgreet (Phosh/phrog removed in V2)
    greetd
    greetd-openrc
    greetd-wlgreet
    
    # Runtime dependencies for Wayland/AtomOS
    gnome-session
    ...
    # XDG portals (we drop xdg-desktop-portal-phosh in V2)
    ...
    # postmarketos-base-ui-gnome (pmaports) parity
    bluez
    bluez-openrc
    udiskie
    dnsmasq
    ...
    zram-init
    zram-init-openrc
    # Compositor runtime dependencies
    libinput
    hwdata
    xkeyboard-config
    libdisplay-info
...
if [ "${ATOMOS_V2:-0}" != "1" ] && [ -f /work/iso-postmarketos/vendor/phoc/meson.build ]; then
...
if [ "${ATOMOS_V2:-0}" != "1" ] && [ -f /work/iso-postmarketos/vendor/phosh-mobile-settings/meson.build ]; then
...
if [ "${ATOMOS_V2:-0}" != "1" ] && [ -f /work/iso-postmarketos/vendor/phosh-wallpapers/meson.build ]; then
...
if [ "${ATOMOS_V2:-0}" = "1" ]; then
    ...
    cat > /target/usr/bin/atomos-session <<EOF_SESSION
#!/bin/sh
...
EOF_SESSION
    ...
    # Rotate QEMU display to portrait for testing
    mkdir -p /target/home/user/.config/cosmic/com.system76.CosmicComp/v1
    cat > /target/home/user/.config/cosmic/com.system76.CosmicComp/v1/outputs <<'EOF_CONFIG'
(
    outputs: {
        "Virtual-1": (
            transform: Flipped90,
...
EOF_CONFIG
    chown -R 10000:10000 /target/home/user/.config
fi
...
if [ "${ATOMOS_V2:-0}" != "1" ] && [ "${BUILD_TOP_BAR:-1}" = "1" ] && [ -f /work/iso-postmarketos/rust/atomos-top-bar/app-gtk/Cargo.toml ]; then
...
EOF
  ...
  echo "Skipping atomos-top-bar Phosh build (V2 session, manifest missing, or BUILD_TOP_BAR=0)."
fi
