...
const SUPPORTED_FEATURES: Features = Features::VERSION_1
    .union(Features::EDID);
...
impl<H: Hal, T: Transport> VirtIOGpu<H, T> {
    ...
    pub fn flush(&mut self) -> Result {
        ...
        info!(
            "virtio-gpu: flush rect={}x{} paddr=0x{:x}",
            rect.width, rect.height,
            self.frame_buffer_dma.as_ref().map(|d| d.paddr()).unwrap_or(0)
        );
        ...
        // Re-assert scanout after flush — some backends need this
        self.set_scanout(rect, SCANOUT_ID, RESOURCE_ID_FB)?;
        info!("virtio-gpu: flush complete");
        Ok(())
    }
    ...
    fn resource_create_2d(&mut self, resource_id: u32, width: u32, height: u32) -> Result {
        info!("GPU CMD: resource_create_2d id={} {}x{} fmt=B8G8R8A8", resource_id, width, height);
        ...
        info!("GPU RSP: resource_create_2d -> {:?}", rsp.hdr_type);
        rsp.check_type(Command::OK_NODATA)
    }

    fn set_scanout(&mut self, rect: Rect, scanout_id: u32, resource_id: u32) -> Result {
        info!("GPU CMD: set_scanout scanout={} resource={} rect={}x{}", scanout_id, resource_id, rect.width, rect.height);
        ...
        info!("GPU RSP: set_scanout -> {:?}", rsp.hdr_type);
        rsp.check_type(Command::OK_NODATA)
    }

    fn resource_flush(&mut self, rect: Rect, resource_id: u32) -> Result {
        info!("GPU CMD: resource_flush id={} rect={}x{}+{}+{}", resource_id, rect.width, rect.height, rect.x, rect.y);
        ...
        info!("GPU RSP: resource_flush -> {:?}", rsp.hdr_type);
        rsp.check_type(Command::OK_NODATA)
    }

    fn transfer_to_host_2d(&mut self, rect: Rect, offset: u64, resource_id: u32) -> Result {
        info!("GPU CMD: transfer_to_host_2d id={} rect={}x{} offset={}", resource_id, rect.width, rect.height, offset);
        ...
        info!("GPU RSP: transfer_to_host_2d -> {:?}", rsp.hdr_type);
        rsp.check_type(Command::OK_NODATA)
    }

    fn resource_attach_backing(&mut self, resource_id: u32, paddr: u64, length: u32) -> Result {
        info!("GPU CMD: resource_attach_backing id={} paddr=0x{:x} len={}", resource_id, paddr, length);
